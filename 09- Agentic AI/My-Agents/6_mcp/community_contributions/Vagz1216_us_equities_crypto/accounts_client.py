...
# Must match MCP wiring: `uv run` resolves scripts from the project root unless cwd + absolute
# script path are set. This folder contains `accounts_server.py` for the crypto/equities variant.
_CONTRIB_DIR = Path(__file__).resolve().parent
params = StdioServerParameters(
    command="uv",
    args=["run", str(_CONTRIB_DIR / "accounts_server.py")],
    cwd=str(_CONTRIB_DIR),
    env=None,
)
