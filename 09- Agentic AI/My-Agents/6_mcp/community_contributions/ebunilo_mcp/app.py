...
class Trader:
    ...
    def get_holdings_df(self) -> pd.DataFrame:
        ...
        if not holdings:
            return pd.DataFrame(columns=["Asset", "Quantity (base)"])
        return pd.DataFrame(
            [{"Asset": sym, "Quantity (base)": qty} for sym, qty in holdings.items()]
        )
    ...
    def get_transactions_df(self) -> pd.DataFrame:
        ...
        if not txs:
            return pd.DataFrame(
                columns=["Timestamp", "Asset", "Qty (base)", "Price USD", "Rationale"]
            )
        rows = []
        for t in txs:
            rows.append(
                {
                    ...
                    "Asset": t.get("base_asset"),
                    "Qty (base)": t.get("quantity_base"),
                    "Price USD": t.get("price_usd"),
        ...
        with gr.Column():
            ...
            with gr.Row():
                self.holdings_table = gr.Dataframe(
                    ...
                    label="Spot holdings (base units)",
                    headers=["Asset", "Quantity (base)"],
                    row_count=(5, "dynamic"),
                    column_count=2,
                    max_height=300,
            ...
            with gr.Row():
                self.transactions_table = gr.Dataframe(
                    ...
                    headers=["Timestamp", "Asset", "Qty (base)", "Price USD", "Rationale"],
                    row_count=(5, "dynamic"),
                    column_count=5,
                    max_height=300,
...
def create_ui():
    ...
    with gr.Blocks(title="Spot crypto (sim)", fill_width=True) as ui:
        with gr.Row():
