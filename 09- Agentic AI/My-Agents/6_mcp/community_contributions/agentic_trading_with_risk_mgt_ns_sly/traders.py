...
async def get_risk_manager(mcp_servers, model_name) -> Agent:
    risk_manager = Agent(
        name="RiskManager",
...
async def get_risk_manager_tool(mcp_servers, model_name) -> Tool:
    risk_manager = await get_risk_manager(mcp_servers, model_name)
    return risk_manager.as_tool(tool_name="RiskManager", tool_description=risk_tool())
...
class Trader:
    ...
    async def create_agent(self, trader_mcp_servers, researcher_mcp_servers) -> Agent:
        ...
        risk_manager_tool = await get_risk_manager_tool(trader_mcp_servers, self.model_name)
        self.agent = Agent(
            name=self.name,
            instructions=trader_instructions(self.name),
            model=get_model(self.model_name),
            tools=[researcher_tool, risk_manager_tool],
