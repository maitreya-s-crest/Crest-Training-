...
class TraderView:
    ...
    def make_ui(self):
        ...
        timer.tick(
            ...
            queue=True,  # Use queue to ensure updates aren't skipped
        )
...
def check_initialization():
    """Check if accounts are initialized with strategies"""
    
    all_initialized = True
    for name in names:
        account = Account.get(name)
        strategy = account.get_strategy()
        has_data = len(account.transactions) > 0 or len(account.holdings) > 0
        
        print(f"\n{name}:")
        print(f"  - Strategy set: {'✓' if strategy else '✗ MISSING'}")
        print(f"  - Has transactions/holdings: {'✓' if has_data else '✗ (no trading data yet)'}")
    ...
    if not all_initialized:
        print("WARNING: Some accounts are missing strategies!")
