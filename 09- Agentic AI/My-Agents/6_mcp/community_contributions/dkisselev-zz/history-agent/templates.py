...
from database import get_simulation_date, get_current_timestamp
...
if is_realtime_polygon:
    ...
    note = "You have access to end of day market data; use your lookup_share_price tool to get the share price as of the prior close."

def researcher_instructions():
    # Check if in simulation mode by reading from database (single source of truth)
...
You are operating in HISTORICAL SIMULATION mode.
The simulated date is: {current_date}

MANDATORY RESEARCH CONSTRAINTS:

1. You can ONLY use information available on or BEFORE {current_date}
2. When searching the web, add "before:{current_date}" to your search queries
3. If you find any articles or data dated AFTER {current_date}, you MUST IGNORE them
4. Verify dates on all sources - discard anything from the future
5. Treat {current_date} as "today" - future information does not exist yet

IMPORTANT: Using future information would invalidate this historical backtest.
You are researching as if you are actually living on {current_date}.
...
The current datetime is {current_date}
"""
